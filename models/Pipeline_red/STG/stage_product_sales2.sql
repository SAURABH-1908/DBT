...
with product as (
    select * from {{ source('RAW_SOURCES','product_2') }}
),

product_group as (
    select * from {{ source('RAW_SOURCES','product_group_2') }}
),

product_sub_group as (
    select * from {{ source('RAW_SOURCES','product_sub_group_2') }}
),

product_line as (
    select * from {{ source('RAW_SOURCES','product_line_2') }}
),
